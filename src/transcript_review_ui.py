...
def get_original_word(original_segment, word_index, fallback_word):
    ...
    # If original transcript is missing or shorter, use fallback.
...
def make_segment_title(segment):
    ...
    # Do not show "Segment", index number or colon.
...
def show_automatic_review_instructions():
    # Show instructions for automatically anonymized word review
    with st.expander("How to review automatically anonymized words", expanded=False):
        st.write(
            "This table shows only the words that were changed automatically by the anonymization step."
        )

        st.write(
            "**Original word** shows what the word was before automatic transcript anonymization."
        )
        ...
        st.write(
            "**Action** lets you decide what should happen to that word."
        )
        ...
        st.write(
            ...
            "A label such as **NIMI**, **SIJAINTI** or **ORGANISAATIO** replaces the word with that label."
        )

        st.write(
            "The **Final word** column shows what will be saved after your selected action. "
            "When everything looks correct, click **Apply automatic anonymization review**."
        )


def show_transcript_review_instructions():
    # Show instructions for full transcript review
    with st.expander("How to review the transcript", expanded=False):
        st.write(
            "Open any transcript segment to read it. "
            "You do not need to review every segment before continuing."
        )

        st.write(
            "Each segment first shows the current anonymized text. "
            "Under it, the original segment text is shown in light grey for comparison."
        )
        ...
        st.write(
            "In edit mode, **Original word** shows the original transcription, "
            "**Anonymized word** shows what will be used in the final transcript, "
            "and **Action** controls whether the word is kept, restored, removed or replaced with a label."
        )
